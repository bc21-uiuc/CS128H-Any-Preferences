...
fn create_blits_file(file_name: &str, sample_rate: u32, bits_per_sample: u16, input: Vec<i32>) -> Result<(), Error> {
    ...
    let lfe_channel_sequence: Vec<ToneBurst> = vec![
        // channel ident
        for i in input {
            ToneBurst::Tone(3.0 * i, 600, -18.0),
        }
    ];
    
    let length = [&lfe_channel_sequence].iter().map(|i| i.duration(sample_rate)).max().unwrap_or(0);

    let frames = (0..=length).map(|frame| {(lfe_channel_sequence.signal(frame, sample_rate, bits_per_sample))});
...
pub fn makeWAV(input:Vec<i32>) -> io::Result<()> {
    ...
    match create_blits_file(&filename, sample_rate, bits_per_sample, input) {
